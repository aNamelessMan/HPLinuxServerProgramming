...
class sort_timer_lst{
    ...
    void adjust_timer(util_timer* timer){
        ...
        //如果需要调整的目标定时器处在链表尾部，或者该定时器新的超时值仍然小于其下一个定时器的超时值，则不用调整
        ...
        //如果目标定时器不是链表的头节点，则将该定时器从链表中取出，然后插入其原来所在位置之后的部分链表中，因为只考虑时间延长的情况
        else{
    ...
    void del_timer(util_timer* timer){
        ...
        //如果链表中至少有两个定时器，且目标定时器是链表的头节点，则将链表的头节点重置为原头节点的下一个节点，然后删除定时器
        if(timer == head){
            head = head->next;
            head->prev = nullptr;
            delete timer;
        ...
        //如果链表中至少有两个定时器，且目标定时器是链表的尾节点，则将链表的尾节点重置为原尾节点的前一个节点，然后删除定时器
        if(timer == tail){
            tail = tail->prev;
            tail->next = nullptr;
            delete timer;
        ...
        //如果目标定时器位于链表的中间，则将它前后的定时器串联起来，然后删除定时器
        timer->prev->next = timer->next;
        timer->next->prev = timer->prev;
        delete timer;
    }
