...
class time_wheel{
    ...
    tw_timer* add_timer(int timeout){
        ...
        如果待插入定时器的超时值小于时间轮的槽间隔SI，则将ticks向上折合为1，否则就将ticks向下折合timeout/SI*/
        if(timeout < SI){
            ticks = 1;
        }else{
            ticks = timeout / SI;
        ...
        //如果第ts个槽中尚无任何定时器(即头节点为空)，则把新建的定时器插入其中，并将该定时器设置为该槽的头节点
        if(!slots[ts]){
    ...
    void del_timer(tw_timer* timer){
        ...
        //slots[ts]是目标都会死去所在槽的头节点，如果目标定时器就是头节点，则需要充值第ts个槽的头节点
        if(timer == slots[ts]){
    ...
    void tick(){
        ...
        while(tmp){
            ...
            //否则，说明定时器已经到期，于是执行定时任务，然后删除该定时器
                ...
                if(tmp == slots[cur_slot]){//如果tmp是所在槽的头节点
